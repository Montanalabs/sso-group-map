...
Untrusted input is an assertion; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an assertion
- **Closed type:** `type Decision = Map(IdpGroup) | Deny` where `IdpGroup` is closed
- **Sink / capability:** `grant mapGroup confidence 70` → `privileged { mapGroup(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/sso-group-map/demo.sh
```
...
- `sso-group-map_safe.wave` — the correct design.
- `sso-group-map_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
